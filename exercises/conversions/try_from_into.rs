...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if (0..=255).contains(&tuple.0) == false {
            return Err("exceed range".to_string()); 
        }
        if (0..=255).contains(&tuple.1) == false {
            return Err("exceed range".to_string()); 
        }
        if (0..=255).contains(&tuple.2) == false {
            return Err("exceed range".to_string()); 
        }
        Ok(Color{red: u8::try_from(tuple.0).unwrap(), green: u8::try_from(tuple.1).unwrap(), blue: u8::try_from(tuple.2).unwrap()})
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() > 3 {
            return Err("excess len".to_string());
        }
        for i in slice.iter() {
            if (0..=255).contains(i) == false {
                return Err("exceed range".to_string());
